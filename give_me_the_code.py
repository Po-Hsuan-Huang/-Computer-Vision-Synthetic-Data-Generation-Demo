...
#--------------------------------------------------------------------------
    ...
    code = C_code + random_space(1,2) + N_code + random_space(1,2)
    #--------------------------------------------------------------------------
...
#    img_flag = img_pers
    img_flag, frequency = flag_transform.transfrom(img)
...
#    oldPos = (code_x-padding, code_y-padding, code_x+padding + code_w, code_y+padding + code_h)
    ...
    for idx in range(1): 
        ...
        if idx < 1: # draw the En 
            ...
            newPos.append( flag_transform.bndbox_transfrom( frequency, oldPos[idx], img))
             
        elif idx == 1:
            oldPos.append((code_x + CodeWidth_En*1.5  , code_y , code_x + CodeWidth_En*1.5 + CodeWidth_Nb, code_y + CodeHeight_Nb ))
            ...
            oldPos.append((code_x + CodeWidth_En*1.5 + (idx-1)*(CodeWidth_Nb) , code_y , code_x + CodeWidth_En*1.5 + (idx-1)*(CodeWidth_Nb) + CodeWidth_Nb, code_y + CodeHeight_Nb ))
...
#%% Crop off tansparent part of the image to fit the image in the frame---
    ...
    paste_size_w = int(min(bg_w, bg_h) * np.random.uniform(0.4,0.7))
...
#    code_h = int( code_h * resize_ratio )
#    code_x = int( code_x * resize_ratio )
#    code_y = int( code_y * resize_ratio )
    
#    img.thumbnail((paste_size_w, paste_size_w), Image.ANTIALIAS)
    img =img_flag
...
#%% Create .xml of image---
#    img_pos =(paste_x, paste_y, paste_x + img_w, paste_y + img_h)
#    text_pos = ( newPos)
#    text_space = 6
#    tree = VOCxml( filename +'.jpg', bg, ('tag','A'), img_pos, text_pos, text_space, font_path )    
...
def give_me_SCC_images(img_num):
    ...
    bg_list2 = glob.glob('./background/bg_train_sets/runners_group/*.jpg')
    bg_list = bg_list + bg_list2
